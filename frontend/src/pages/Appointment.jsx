...
const Appointment = () => {
    ...
    const getAvailableSlots = async () => {
        ...
        console.log("Doc info in slot generation:", docInfo);
        const allSlots = [];
        let today = new Date();
        ...
        for (let i = 0; i < 7; i++) {
            ...
            console.log(`Day ${i} slots:`, timeSlots);
        }
                ...
                <div className="flex items-center gap-3 w-full overflow-x-scroll mt-4 no-scrollbar">
                    {docSlots.length > 0 && docSlots[slotIndex]?.map((item, index) => (
                        <p
                            ...
                            className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer border ${item.time === slotTime ? 'bg-primary text-white' : 'text-gray-400 border-gray-300'}`}
                        >
